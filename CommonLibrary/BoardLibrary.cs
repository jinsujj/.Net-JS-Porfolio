     ...
     string file)
        {
            if (strFileName.Length <= 0)
                return "-";
            return "<a href=\"/" + folder + file + "?Id=" + id.ToString() + "\">" + BoardLibrary.DownloadType(strFileName, strFileName + "(" + BoardLibrary.ConvertToFileSize(Convert.ToInt32(strFileSize)) + ")") + "</a>";
        }
        #endregion
